...
The first idea and code, with some variations, left me with some
overlapping ranges. I don't think Python's "for i in iterator"
completely works here, without some extra variables to track a
previous item. So add a previous item variable and coalesce
overlapping and adjacent ranges based on previous and current.

**** Other idea

- Use indices. Very non-Pythonic.
- Use iter(ranges) and next(). I don't know if I need this yet.
...
def add_range(line):
    """
    Parse a line into start and end and add that range to global ranges.
    line should look like

    start-end

    Parse into a list of two integers. Append that list to ranges.
    """
...
One thing I've noticed is that modifying a list while iterating over
it seems to skip over some elements. Note to use of a copy of ranges
while iterating, but modifying and removing items in the actual ranges.
...
def coalesce():
    ...
    previous = None             # previous range

    ranges = sorted(ranges)
    for r in ranges[:]:
        if previous is None:    # can't coalesce yet
            previous = r
            continue
        ...
        if start <= prev_end or start == prev_end+1:
            # coalesce prev into r
            r[0] = prev_start
            r[1] = max(prev_end, end)
...
def five_b():
    ...
    # Count the ranges. The number of ingredients in a range is end - start + 1.
    count = 0
    for r in ranges:
        count += r[1] + 1 - r[0]
